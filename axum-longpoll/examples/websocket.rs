...
use sio_rs2::transport::Transport;
...
async fn ws_handler(
    ...
    ws.on_connect(move |socket| handle_socket(socket, addr))
}
...
async fn handle_socket(mut socket: DuplexSession, who: SocketAddr) {
    ...
    if let Some(msg) = socket.recv().await {
        if let Ok(msg) = msg {
            if process_message(msg.into(), who).is_break() {
                return;
            }
        } else {
    ...
    let mut send_task = tokio::spawn(async move {
        ...
        for i in 0..n_msg {
            ...
            if sender
                .send(Message::Text(format!("Server message {i} ...").into()).into())
                .await
                .is_err()
            {
                return i;
